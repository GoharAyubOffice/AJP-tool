...
class WorkExperience(BaseModel):
    ...
    def validate_end_date(cls, v: str) -> str:
        ...
        if len(parts) != 2:
            raise ValueError(
                f"endDate must be in YYYY-MM format or 'Present', got: {v}"
            )
        year, month = parts
        if not (year.isdigit() and len(year) == 4):
...
class Application(BaseModel):
    ...
    status: Literal["pending", "submitted", "rejected", "interview", "offer"] = (
        "pending"
    )
...
class TailoredCV(BaseModel):
    ...
    fewer work experiences, different bullet ordering, a customised
    personal statement, and tailored projects list.
    """
